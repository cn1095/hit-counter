...
func getFontDrawer(fontType FontType) (fontDrawer, error) {
	...
	default:
		return nil, perrors.WithStack(fmt.Errorf("[err] not found font type %v", fontType))
	}
}

func init() {
	// initialize vera sans font drawer.
	veraTTF, err := assets.VeraSans.ReadFile("vera_sans/Vera.ttf")
	if err != nil {
	...
	verdanaTTF, err := assets.Verdana.ReadFile("verdana/verdana.ttf")
	if err != nil {
		panic(perrors.WithStack(err))
	}
